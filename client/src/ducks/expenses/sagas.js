...
function* fetchExpensesSaga() {
  try {
    const token = yield select(state => state.auth.user.token);
    const response = yield call(ApiService, `expenses?$sort[datetime]=1`, 'GET', null, token);
...
function* fetchAllExpensesSaga() {
  try {
    const token = yield select(state => state.auth.user.token);
    const response = yield call(ApiService, `expenses?all=true&$sort[datetime]=1`, 'GET', null, token);
